	...
	public Product getProduct(int pid) {
		...
		try {
			...
			Connection connectionToDB = DriverManager.getConnection(
					"jdbc:mysql://localhost:3306/ecommerce?createDatabaseIfNotExist=true", "root", "poiuy@098");
			Statement st = connectionToDB.createStatement();
			// 2-Execute a query to get data from the database
			ResultSet resultData = st.executeQuery("select * from product where id=" + pid + ";");
	...
	public Product saveProduct(int id, String sname, String scompany) {
		...
		try {
			...
			Connection connection = DriverManager.getConnection(
					"jdbc:mysql://localhost:3306/ecommerce?createDatabaseIfNotExist=true", "root", "Abc@123");
